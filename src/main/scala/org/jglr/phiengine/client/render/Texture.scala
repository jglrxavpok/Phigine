...
object Texture {
  def createFrom(img: BufferedImage): Texture = {
    new Texture(null, img)
  }
}

class Texture(pointer: FilePointer, _img: BufferedImage = null) extends ReferenceCounter with IDisposable with ITexture {
  ...
  def this(classpath: String) {
    this(new FilePointer(classpath))
  }
}